...
export class CommunityService {
  ...
  async createPost(userId: string, dto: CreatePostDto): Promise<DecoratedCommunityPostRow> {
    const supabase = this.db()

    const destinationId =
      dto.destinationId ?? (await this.findDestinationIdByName(dto.destinationName, dto.province))
  ...
  async uploadPostMedia(
    ...
    const mediaType: MediaType = file.mimetype.startsWith('image/') ? 'image' : 'video'
    const maxSize = mediaType === 'image' ? this.maxImageSize : this.maxVideoSize
    ...
    const { data: publicUrlData } = supabase.storage.from(this.bucketName).getPublicUrl(filePath)
  ...
  async getPosts(userId?: string, filter = 'all'): Promise<DecoratedCommunityPostRow[]> {
    const supabase = this.db()
    ...
    if (normalizedFilter === 'following') {
      ...
      feedItems = feedItems.filter((post) => friendIds.includes(this.feedOwnerId(post)))
    }
    ...
    if (normalizedFilter === 'saved') {
      ...
      feedItems = feedItems.filter((post) => {
        return (post.saved_posts || []).some((saved) => saved.user_id === userId)
      })
    }

    if (normalizedFilter === 'popular') {
      feedItems = feedItems.sort((a, b) => {
        const scoreA = (a.post_likes?.length || 0) * 2 + (a.post_comments?.length || 0)

        const scoreB = (b.post_likes?.length || 0) * 2 + (b.post_comments?.length || 0)
  ...
  async deletePost(userId: string, postId: string) {
    ...
    if (postId.startsWith('share:')) {
      const shareId = postId.substring(6)

      const { data: share, error: findError } = await (supabase as any)
        .from('shared_posts')
        .select('id, user_id')
        .eq('id', shareId)
        .maybeSingle()
  ...
  async updatePost(
    ...
    if (postId.startsWith('share:')) {
      ...
      if (share.user_id !== userId) {
        throw new ForbiddenException('You do not have permission to modify this shared post')
      }

      const { data: updatedShare, error: updateError } = await (supabase as any)
        .from('shared_posts')
        .update({ caption: dto.caption || null })
        .eq('id', shareId)
        .select(
          `
          id,
      ...
      if (updateError || !updatedShare) {
        throw new BadRequestException(updateError?.message || 'Failed to update shared post')
      }

      const feedRow = this.mapSharedPostFeedRow(updatedShare as SharedPostRow)
      if (!feedRow) {
        throw new BadRequestException('Failed to load updated shared post')
      }
    ...
    if (updateError || !updatedPost) {
      throw new BadRequestException(updateError?.message || 'Failed to update post')
    }

    return this.decoratePostForViewer(updatedPost as unknown as CommunityPostRow, userId)
  }
  ...
  async updatePostVisibility(
    ...
    if (error || !data) {
      throw new BadRequestException(error?.message || 'Failed to update visibility')
    }

    return this.decoratePostForViewer(data as unknown as CommunityPostRow, userId)
  }
  ...
  async createComment(userId: string, postId: string, dto: CreateCommentDto): Promise<unknown> {
    const supabase = this.db()
  ...
  private async findDestinationIdByName(destinationName?: string, province?: string) {
    if (!destinationName) {
      return null
    }
    ...
    let query = supabase.from('destinations').select('id').ilike('name', destinationName).limit(1)
