  ...
  >
    ...
    <div class="relative h-[220px] sm:h-[240px] overflow-hidden">
      ...
      <div v-if="destination.badge" class="absolute top-3 left-3">
        <span
          :class="[
            ...
            destination.badge === 'Trending' ? 'bg-emerald-600/85' : 'bg-purple-700/80',
          ]"
        >
          <svg
      ...
      <div class="absolute bottom-0 left-0 right-0 p-4 flex items-end justify-between">
        <div class="min-w-0">
          <h3 class="text-[16px] sm:text-[17px] font-bold text-white m-0 mb-0.5 drop-shadow-md">
            {{ destination.name }}
          </h3>
        ...
        <div
          v-if="destination.rating != null"
          class="flex items-center gap-1 bg-black/30 backdrop-blur-sm px-2 py-1 rounded-lg shrink-0"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="13"
