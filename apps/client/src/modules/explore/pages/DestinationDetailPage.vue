<template>
  <main v-if="destination" class="min-h-screen bg-cream">
    ...
    <div class="relative h-[400px] sm:h-[500px] overflow-hidden">
      ...
      />
      <div v-else class="w-full h-full bg-gradient-to-br from-slate-300 to-slate-400 flex items-center justify-center">
        <svg class="text-slate-500" xmlns="http://www.w3.org/2000/svg" width="80" height="80" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="18" x="3" y="3" rx="2" ry="2"/><circle cx="9" cy="9" r="2"/><path d="m21 15-3.086-3.086a2 2 0 0 0-2.828 0L6 21"/></svg>
      </div>
      ...
      <div class="absolute bottom-0 left-0 right-0 p-6 sm:p-8">
        <div class="flex items-end justify-between">
          <div>
            ...
            <h1 class="text-[32px] sm:text-[42px] font-bold text-white m-0 mb-2">{{ destination.name }}</h1>
          ...
          <div v-if="destination.rating" class="flex items-center gap-2 bg-black/40 backdrop-blur-sm px-4 py-2 rounded-xl">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="#FBBF24" stroke="#FBBF24" stroke-width="1"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        <div class="bg-white p-6 rounded-2xl border border-weather-border shadow-sm">
          <p class="text-[13px] font-bold text-slate-500 uppercase tracking-wide mb-3 flex items-center gap-2">
        ...
        <div class="bg-white p-6 rounded-2xl border border-weather-border shadow-sm">
          <p class="text-[13px] font-bold text-slate-500 uppercase tracking-wide mb-3 flex items-center gap-2">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="1"/><path d="M12 1v6m6.16-1.86-4.24 4.24M19 12h-6m1.86 6.16-4.24-4.24M12 19v-6m-6.16 1.86 4.24-4.24M5 12H11M6.16 6.16l4.24 4.24"/></svg>
            Best Time
          </p>
