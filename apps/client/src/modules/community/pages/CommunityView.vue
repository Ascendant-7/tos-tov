  ...
  <div class="py-5 px-4 sm:py-6 sm:px-6 md:py-8 md:px-8 max-w-[900px] mx-auto">
      ...
      >
        ...
        <div class="flex-shrink-0 w-[72px] sm:w-[80px] snap-start">
          ...
          >
            ...
            />
            <p class="text-[10px] sm:text-[11px] font-semibold text-slate-600 mt-1 text-center">
              Your Story
            </p>
          </button>
        </div>
        ...
        <!-- Story Circles -->
        <div
          v-for="(user, i) in communityStore.communityUsers"
          :key="user.id"
        ...
        >
          ...
          >
            ...
            <div
              v-if="user.hasStory"
              class="absolute inset-0 border-2 border-accent-gold rounded-2xl pointer-events-none"
            ></div>
          </button>
          <p class="text-[10px] sm:text-[11px] text-slate-600 text-center mt-1.5 truncate">
            {{ user.name.split(' ')[0] }}
          </p>
        </div>
      </div>
    ...
    <section class="mb-6 sm:mb-8 animate-fade-in-up delay-2">
      ...
      <div class="flex gap-2 flex-wrap">
        <button
          v-for="tag in communityStore.trendingTags"
          :key="tag"
          @click="communityStore.setSelectedTag(tag)"
          :class="[
    ...
    <section class="mb-6 sm:mb-8 animate-fade-in-up delay-3">
      <div class="flex gap-2 sm:gap-3">
        <button
          v-for="tab in communityStore.filterTabs"
          :key="tab"
          @click="communityStore.setActiveFilter(tab)"
          :class="[
    ...
    <section class="animate-fade-in-up delay-4">
      <div v-if="communityStore.filteredPosts.length > 0" class="space-y-4 sm:space-y-6">
        <div v-for="(post, i) in communityStore.filteredPosts" :key="post.id">
          <CommunityPostCard
            ...
            @like="communityStore.toggleLike(post.id)"
            @bookmark="communityStore.toggleBookmark(post.id)"
            @share="emit('share-post', post)"
            @comment="emit('toggle-comments', post)"
            @update:comment-draft="emit('update:new-comment', $event)"
      ...
      >
        ...
        <p class="text-[12px] text-slate-400 m-0">
