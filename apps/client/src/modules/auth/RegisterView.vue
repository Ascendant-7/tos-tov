<template>
  ...
  >
    ...
    <div
      class="w-full max-w-[1100px] grid md:grid-cols-[1.1fr_0.9fr] rounded-[28px] overflow-hidden shadow-[0_30px_80px_rgba(38,37,35,0.25)] bg-white/95 backdrop-blur"
    >
      <!-- LEFT PANEL -->
      <aside
      ...
      >
        ...
        <div>
          ...
          <h2 class="text-3xl font-serif mb-3">Meet the world halfway.</h2>
        ...
        <!-- Decorations -->
        <div
          class="absolute -right-32 -bottom-32 w-80 h-80 rounded-full bg-gradient-to-br from-orange-300 to-orange-500 opacity-70"
        ></div>

        <div
          class="absolute left-[-80px] top-1/2 w-60 h-60 rounded-full border border-white/30"
        ></div>
      </aside>
      ...
      <main class="p-10 md:p-14 flex flex-col gap-6 bg-white/95">
        <div>
          <span class="uppercase text-xs tracking-widest text-[#0b6f91] font-semibold">
            Create account
          </span>
        ...
        <form class="flex flex-col gap-5" @submit.prevent="handleSubmit">
          ...
          <button
            type="submit"
            :disabled="loading"
            class="bg-gradient-to-r from-[#0b5678] to-[#2aa8d1] text-white py-4 rounded-xl font-semibold hover:-translate-y-0.5 transition flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
          >
          ...
          <div class="relative text-center text-sm text-gray-400 my-2">
            <span class="bg-white px-2 relative z-10"> or sign up with </span>
        ...
        <p class="text-sm text-center">
          ...
          <router-link to="/login" class="text-blue-600 font-medium"> Log in </router-link>
        </p>
      </main>
    </div>
  </div>
